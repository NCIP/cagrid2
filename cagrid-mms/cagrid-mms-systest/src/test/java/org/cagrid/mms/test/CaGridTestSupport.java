...
public abstract class CaGridTestSupport {
	...
	// public static final String WSRF_FEATURE_VERSION_PROPERTY =
	// "cagrid.wsrf.version";
	...
	public void setUp() {
		...
		System.err
				.println(executeCommand("features:addurl "
						+ maven()
								.groupId("org.cagrid")
								.artifactId("cagrid-features")
								.version(
										System.getProperty(CAGRID_FEATURE_VERSION_PROPERTY))
								.classifier("features").type("xml").getURL()));
	...
	public Option[] config() {
		...
		return new Option[] {
				...
				// systemProperty(WSRF_FEATURE_VERSION_PROPERTY,
				// MavenUtils.getArtifactVersion("org.cagrid.wsrf",
				// "wsrf-draft-features")),
				systemProperty(CAGRID_FEATURE_VERSION_PROPERTY,
						MavenUtils.getArtifactVersion("org.cagrid",
								"cagrid-features")),
				// Pass local/private maven locations into the forked JVMs (this
				// is needed if you use jenkins with a per-build maven repo)
				when(System.getProperty("maven.repo.local") != null)
						.useOptions(
								vmOption("-Dorg.ops4j.pax.url.mvn.localRepository="
										+ System.getProperty("maven.repo.local"))),
				when(
						System.getProperty("org.ops4j.pax.url.mvn.localRepository") != null)
						.useOptions(
								vmOption("-Dorg.ops4j.pax.url.mvn.localRepository="
										+ System.getProperty("org.ops4j.pax.url.mvn.localRepository"))),
				// servicemix defaults to using the felix osgi kernal which has
				// some bugs when dealing with complex feature/bundle
				// dependencies, so let's use
				// equinox by default
				new KarafDistributionConfigurationFilePutOption(
						"etc/config.properties", // config file to modify based
													// on karaf.base
						"karaf.framework", // key to add or change
						"equinox") // value to add or change
				,
				new KarafDistributionConfigurationFileExtendOption(
						"etc/org.apache.karaf.features.cfg", // config file to
																// modify based
																// on karaf.base
		...
		return karafDistributionConfiguration()
				.frameworkUrl(
						maven().groupId("org.apache.servicemix")
								.artifactId("apache-servicemix").type("tar.gz")
								.versionAsInProject())
				.karafVersion(getKarafVersion()).name("Apache Servicemix")
	...
	protected <T> T getOsgiService(Class<T> type, String filter, long timeout) {
		...
		try {
			...
			Object srvInst = tracker.waitForService(timeout);
			System.out.println("ABOUT TO CAST " + srvInst.getClass().getName()
					+ " to " + type.getName());
			Object svc = null;
			svc = type.cast(srvInst);
			if (svc == null) {
	 ...
	 * @throws Exception
	 */
	public void installAndAssertFeature(String feature, Long timeout)
			throws Exception {
	...
	protected void assertBundleInstalled(String name) {
		Assert.assertTrue("Bundle " + name + " should be installed",
				isBundleInstalled(name));
	}

	protected void assertBundleNotInstalled(String name) {
		Assert.assertFalse("Bundle " + name + " should not be installed",
				isBundleInstalled(name));
	}
	...
	protected String getKarafVersion() {
		// karaf is a transitive dependency of SMX, so we can pull the version
		// thusly
	...
	public static Option systemProperty(String propertyName,
		...
		return editConfigurationFileExtend("etc/system.properties",
				propertyName, propertyValue != null ? propertyValue : "");
	}
