...
public class ServiceDomainModelGenerationTestCase extends MMSAbstractBaseTestCaseBase {
	...
	@Test
	 public void test() {
	        try {
				...
				try {
					domainModel = mms.generateDomainModelForPackages(project,
					    (List<String>)Arrays.asList(new String[]{"gov.nih.nci.cabio.domain"}));
...
//	                + "_DomainModel.xml");
	            ...
	            //JAXBElement<DomainModel> element = new JAXBElement<DomainModel>(MetadataConstants.CAGRID_DATA_MD_QNAME, domainModel.getClass(), domainModel);
	            JAXBContext jaxbContext = JAXBContext.newInstance(domainModel.getClass());
	    		JAXBElement<DomainModel> el = new ObjectFactory().createDomainModel(domainModel);
	    ...
	    private static void replaceIllegalCharacters(DomainModel domainModel) {
	        ...
	        UMLClass[] classes = (UMLClass[])classColl.getUMLClass().toArray(new UMLClass[0]);
	        for (UMLClass klass : classes) {
	            ...
	            UMLAttribute[] atts = (UMLAttribute[])attColl.getUMLAttribute().toArray(new UMLAttribute[0]);
	            for (UMLAttribute att : atts) {
	                ...
	                att.getSemanticMetadata().addAll(Arrays.asList((fixSemanticMetadata(sb, klass.getClassName() + "." + att.getName(), (SemanticMetadata[])att
	                    .getSemanticMetadata().toArray(new SemanticMetadata[0]), badChars))));
	            }
	            klass.getSemanticMetadata().addAll(Arrays.asList(fixSemanticMetadata(sb, klass.getClassName(), (SemanticMetadata[])klass.getSemanticMetadata().toArray(new SemanticMetadata[0]),
	                badChars)));
	        }
