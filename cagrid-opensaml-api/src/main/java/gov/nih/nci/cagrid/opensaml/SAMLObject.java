...
{
	private final static Logger sLogger = LoggerFactory.getLogger(SAMLObject.class);
    ...
    static protected Element fromStream(InputStream in) throws SAMLException {
        ...
        {
            sLogger.error("SAMLObject.fromStream() caught exception while parsing a stream",e);
    ...
    public void toStream(OutputStream out) throws java.io.IOException, SAMLException {
        try
        {
            ...
            out.write(c.canonicalizeSubtree(root, config.getProperty("gov.nih.nci.cagrid.opensaml.inclusive-namespace-prefixes")));
        }
    ...
    public String toString() {
        ...
        {
            log.error("toString(): caught an I/O exception while serializing XML: " + e);
            return "";
        }
        catch (SAMLException e)
        {
            log.error("toString(): caught a SAML exception while serializing XML: " + e);
            return "";
        }
    }
}
