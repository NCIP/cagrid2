...
public class Upgrader {
	...
	public void upgrade(boolean trialRun) {
		try {
			...
			if (upgrades.size() == 0) {
				System.out.println("No upgrades required, Dorian is already upgraded to the latest version (" + PropertyManager.CURRENT_VERSION + ").");
			} else {
				System.out.println("Attempting to upgrade Dorian from version " + properties.getVersion() + " to version " + PropertyManager.CURRENT_VERSION + ".");
				for (int i = 0; i < upgrades.size(); i++) {
					Upgrade u = upgrades.get(i);
					if (!properties.getVersion().equals(u.getStartingVersion())) {
						if (!trialRun) {
							throw new Exception("Cannot run the upgrader " + u.getClass().getName() + ", this upgrader starts with " + u.getStartingVersion() + ", your system is using version "
									+ properties.getVersion() + ".");
						}
					}

					System.out.println("Attempting to run upgrader " + u.getClass().getName() + " which upgrades from Dorian " + u.getStartingVersion() + " to Dorian " + u.getUpgradedVersion() + ".");
					u.upgrade(trialRun);

					if (!trialRun) {
						System.out.println("Dorian upgraded from version " + u.getStartingVersion() + " to version " + u.getUpgradedVersion() + ".");
					}
	...
	public static void main(String[] args) {
		...
		Option conf = new Option(DORIAN_CONFIG_FILE_OPT, DORIAN_CONFIG_FILE_FULL, true, "The config file for the Dorian CA.");
		...
		Option props = new Option(DORIAN_PROPERTIES_FILE_OPT, DORIAN_PROPERTIES_FILE_FULL, true, "The properties file for the Dorian CA.");
		props.setRequired(true);
		options.addOption(props);

		Option uconf = new Option(UPGRADER_CONFIG_FILE_OPT, UPGRADER_CONFIG_FILE_FULL, true, "The config file for the Dorian Upgrader.");
		...
		try {
			...
			if (line.getOptionValue(HELP_OPT) != null) {
				...
				BeanUtils beanUtils = new BeanUtils(new FileSystemResource(dorianConfigFile), new FileSystemResource(dorianPropertiesFile));
