package org.cagrid.dorian.service;

public class DorianConstants {
	public static final String DORIAN_BEAN = "dorian";
    public static final String DATABASE_BEAN = "database";
    public static final String CA_BEAN = "certificateAuthority";
    public static final String CA_MANAGER_BEAN = "certificateAuthorityManager";
    public static final String LEGACY_CA_PROPERTIES_BEAN = "legacyCAProperties";
    public static final String CA_PROPERTIES_BEAN = "caProperties";
    public static final String IDP_USER_MANAGER_BEAN = "idpUserManager";
    public static final String IDP_PROPERTIES_BEAN = "identityProviderProperties";
    public static final String IDP_BEAN = "identityProvider";
    public static final String IDP_ASSERTION_MANAGER_BEAN = "assertionCredentialsManager";
    public static final String IDENTITY_FEDERATION_PROPERTIES_BEAN = "identityFederationProperties";
    public static final String TRUSTED_IDP_MANAGER_BEAN = "trustedIdPManager";
    public static final String DORIAN_PROPERTIES_BEAN = "dorianProperties";
    public static final String EVENT_MANAGER_BEAN = "eventManager";
    public static final String ANONYMOUS_CALLER = "<anonymous>";
}
