...
import org.cagrid.dorian.model.exceptions.DorianInternalFault;
...
public class TestAssertionCredentialsManager extends TestCase {
	...
	public void testAutoCredentialCreationRenew() {
		...
		try {
			...
			X509Certificate shortCert = ca.signCertificate(subject, pair.getPublic(), start, end, CertificateSignatureAlgorithm.SHA2);
	...
	public void testAutoCredentialCreationNoRenewal() {
		...
		try {
			...
			try {
				...
				assertTrue(false);
			} catch (DorianInternalException fault) {
