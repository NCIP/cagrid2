...
public class TestHostCertificateManager extends TestCase implements Publisher {
	...
	public void testCreateDuplicateHostCertificate() {
		try {
			...
			try {
				...
				fail("Should have Failed!!");
			} catch (InvalidHostCertificateRequestException f) {
	...
	private void validateAfterCertificateRequest(HostCertificateManager hcm, HostCertificateRequest req, long id) throws Exception {
		validateAfterCertificateRequest(1, 1, hcm, OWNER, req, id);
	}

	private void validateAfterCertificateRequest(int count, int statusCount, HostCertificateManager hcm, String owner, HostCertificateRequest req, long id) throws Exception {
		validateFindHostCertificates(count, statusCount, hcm, id, -1, null, req.getHostname(), owner, req.getPublicKey(), HostCertificateStatus.PENDING, "");
	}

	private void validateAfterCertificateApproval(HostCertificateManager hcm, long id, String owner, HostCertificateRequest req, HostCertificateRecord record) throws Exception {
		validateAfterCertificateApproval(1, 1, hcm, id, owner, req, record);
	}

	private void validateAfterCertificateApproval(int count, int statusCount, HostCertificateManager hcm, long id, String owner, HostCertificateRequest req, HostCertificateRecord record)
			throws Exception {
		assertEquals(req.getHostname(), record.getHost());
		assertEquals(req.getPublicKey().getKeyAsString(), record.getPublicKey().getKeyAsString());
		assertEquals(owner, record.getOwner());
		assertEquals(HostCertificateStatus.ACTIVE, record.getStatus());
		String subject = org.cagrid.dorian.service.util.Utils.getHostCertificateSubject(ca.getCACertificate(), req.getHostname());
		assertEquals(subject, record.getSubject());
		HostCertificateRecord r = hcm.getHostCertificateRecord(id);
		assertEquals(record.getPublicKey().getKeyAsString(), r.getPublicKey().getKeyAsString());
		assertEquals(record.getCertificate().getCertificateAsString(), r.getCertificate().getCertificateAsString());
		...
		validateFindHostCertificates(count, statusCount, hcm, id, record.getSerialNumber(), record.getCertificate(), req.getHostname(), owner, req.getPublicKey(), HostCertificateStatus.ACTIVE,
				subject);
	}

	private void validateFindHostCertificates(int count, int statusCount, HostCertificateManager hcm, long id, long sn, X509Certificate cert, String host, String owner, PublicKey key,
			HostCertificateStatus status, String subject) throws Exception {
	...
	private void validateHostCertificateRecord(HostCertificateRecord record, long id, long sn, X509Certificate cert, String host, String owner, PublicKey key, HostCertificateStatus status,
			String subject) {
		...
		assertEquals(owner, record.getOwner());
		assertEquals(key.getKeyAsString(), record.getPublicKey().getKeyAsString());
		assertEquals(status, record.getStatus());
		assertEquals(subject, record.getSubject());
	}

	public void setHostCertificateStatus(HostCertificateManager hcm, long id, HostCertificateStatus status) throws Exception {
		HostCertificateUpdate update = new HostCertificateUpdate();
		update.setId(id);
