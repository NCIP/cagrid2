...
public class PasswordSecurityManager {
	...
	private final static Logger logger = LoggerFactory
			.getLogger(PasswordSecurityManager.class);
	...
	public synchronized PasswordSecurity getEntry(String uid,
			boolean includeSalt) throws DorianInternalException {
		this.buildDatabase();
		if (!entryExists(uid)) {
			insertEntry(uid);
		}
		...
		try {
			...
			if (rs.next()) {
				...
				if (entry.getTotalInvalidLogins() >= policy
					...
					entry.setPasswordStatus(PasswordStatus.LOCKED_UNTIL_CHANGED);
				} else {
		...
		if (entry == null) {
			DorianInternalException fault = FaultHelper.createFaultException(
					DorianInternalException.class,
					"An unexpected error occurred in locating the password security entry for "
							+ uid + ".");
			throw fault;
		}
	...
	public void resetEntry(String uid, String salt)
			throws DorianInternalException {
		PasswordSecurity entry = getEntry(uid);
		entry.setConsecutiveInvalidLogins(0);
	...
	public synchronized void deleteEntry(String uid)
			throws DorianInternalException {
		this.buildDatabase();
		Connection c = null;
		try {
	...
	public static String encrypt(String password, String salt) throws Exception {
		...
		MessageDigest digest = MessageDigest
				.getInstance(PASSWORD_DIGEST_ALGORITHM);
		digest.reset();
