...
public class EracomCertificateAuthority extends CertificateAuthority implements
		WrappingCertificateAuthority {

	private final static Logger logger = LoggerFactory
			.getLogger(EracomCertificateAuthority.class);
	...
	public EracomCertificateAuthority(
	...
	public void deleteCACredentials() throws CertificateAuthorityException {
		try {
			...
			CertificateAuthorityException fault = FaultHelper
					.createFaultException(CertificateAuthorityException.class,
							"Unexpected Error, could not delete the CA credentials.");
			FaultHelper.addMessage(fault, e.getMessage());
			throw fault;
		}
	...
	protected X509Certificate getCertificate()
		...
		try {
			if (!hasCACredentials()) {
				...
				return convert((X509Certificate) getKeyStore().getCertificate(
						CA_ALIAS));
			}
	...
	public boolean hasCACredentials() throws CertificateAuthorityException {
		try {
			...
			CertificateAuthorityException fault = FaultHelper
					.createFaultException(CertificateAuthorityException.class,
							"An unexpected error occurred, could determin if credentials exist.");
			FaultHelper.addMessage(fault, e.getMessage());
			throw fault;
		}
	}
	...
	public void setCACredentials(X509Certificate cert, PrivateKey key,
		...
		try {
			getKeyStore().setKeyEntry(CA_ALIAS, key, null,
					new X509Certificate[] { cert });
		} catch (Exception e) {
			logger.error(e.getMessage(), e);
			CertificateAuthorityException fault = FaultHelper
					.createFaultException(CertificateAuthorityException.class,
							"Unexpected Error, could not store CA credentials.");
			FaultHelper.addMessage(fault, e.getMessage());
			throw fault;
		}
	}
