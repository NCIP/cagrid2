...
public abstract class CertificateAuthority {
	...
	public abstract boolean hasCACredentials()
			throws CertificateAuthorityException;

	public abstract void setCACredentials(X509Certificate cert, PrivateKey key,
			String password) throws CertificateAuthorityException;

	public abstract void deleteCACredentials()
			throws CertificateAuthorityException;

	public abstract PrivateKey getPrivateKey(String password)
			throws CertificateAuthorityException, NoCACredentialsException;

	protected abstract java.security.cert.X509Certificate getCertificate()
			throws CertificateAuthorityException;
		...
		try {
			...
			CertificateAuthorityException fault = FaultHelper
					.createFaultException(CertificateAuthorityException.class,
							"Unexpected Error, could not initialize the Dorian Certificate Authority.");
	...
	private void createCertifcateAuthorityCredentials(String dn,
			Date expirationDate, int keySize)
			throws CertificateAuthorityException, NoCACredentialsException {
		try {
			...
			this.setCACredentials(cacert, pair.getPrivate(),
					properties.getCertificateAuthorityPassword());
		} catch (Exception e) {
			logger.error(e.getMessage(), e);
			CertificateAuthorityException fault = FaultHelper
					.createFaultException(CertificateAuthorityException.class,
							"Unexpected Error, could not create the CA credentials.");
			FaultHelper.addMessage(fault, e.getMessage());
			throw fault;
		}
	}
	...
	private java.security.cert.X509Certificate getCACertificate(
		...
		try {
			if (!hasCACredentials()) {
				NoCACredentialsException fault = FaultHelper
						.createFaultException(NoCACredentialsException.class,
								"No certificate exists for the CA.");
				throw fault;
			} else {
				cert = getCertificate();
			}
			...
			CertificateAuthorityException fault = FaultHelper
					.createFaultException(CertificateAuthorityException.class,
							"Unexpected Error, Error obtaining the CA private key.");
			FaultHelper.addMessage(fault, e.getMessage());
			throw fault;
		}
				...
				if (properties.isAutoRenewCAEnabled()) {
					...
					NoCACredentialsException fault = FaultHelper
							.createFaultException(
									NoCACredentialsException.class,
									"The CA certificate had expired or is not valid at this time.");
					throw fault;
				}
			}
		}
	...
	public synchronized X509Certificate signCertificate(String subject,
			PublicKey publicKey, Date start, Date expiration)
			throws CertificateAuthorityException, NoCACredentialsException {
		init();
		...
		if (start.after(caDate)) {
			CertificateAuthorityException fault = FaultHelper
					.createFaultException(CertificateAuthorityException.class,
							"Certificate start date is after the CA certificates expiration date.");
			throw fault;
		}
		if (expiration.after(caDate)) {
			CertificateAuthorityException fault = FaultHelper
					.createFaultException(CertificateAuthorityException.class,
							"Certificate expiration date is after the CA certificates expiration date.");
			throw fault;
		}
		...
		try {
			...
			CertificateAuthorityException fault = FaultHelper
					.createFaultException(CertificateAuthorityException.class,
							"Unexpected Error, could not sign certificate.");
			FaultHelper.addMessage(fault, e.getMessage());
			throw fault;
		}
	}

	public synchronized X509Certificate signHostCertificate(String host,
			PublicKey publicKey, Date start, Date expiration)
			throws CertificateAuthorityException, NoCACredentialsException {
		init();
		X509Certificate cacert = getCACertificate();
		try {
			...
			CertificateAuthorityException fault = FaultHelper
					.createFaultException(CertificateAuthorityException.class,
							"Unexpected Error, could not sign host certificate.");
			FaultHelper.addMessage(fault, e.getMessage());
			throw fault;
		}
	}
	...
	public synchronized X509Certificate renewCertifcateAuthorityCredentials(
			Date expirationDate) throws CertificateAuthorityException,
			NoCACredentialsException {
		init();
		try {
			...
			CertificateAuthorityException fault = FaultHelper
					.createFaultException(CertificateAuthorityException.class,
							"Unexpected Error, could renew the CA credentials.");
			FaultHelper.addMessage(fault, e.getMessage());
			throw fault;
		}
	}

	public X509CRL getCRL(CRLEntry[] entries)
			throws CertificateAuthorityException, NoCACredentialsException {
		try {
			init();
			return CertUtil.createCRL(getCACredentialsProvider(),
					getCACertificate(), getPrivateKey(), entries,
					getCACertificate().getNotAfter(), getSignatureAlgorithm());
		} catch (Exception e) {
			logger.error(e.getMessage(), e);
			CertificateAuthorityException fault = FaultHelper
					.createFaultException(CertificateAuthorityException.class,
							"Unexpected Error, could not create the CRL.");
			FaultHelper.addMessage(fault, e.getMessage());
			throw fault;
		}
	}

	public PrivateKey getPrivateKey() throws CertificateAuthorityException,
			NoCACredentialsException {
		init();
		try {
			if (!hasCACredentials()) {
				NoCACredentialsException fault = FaultHelper
						.createFaultException(NoCACredentialsException.class,
								"No Private Key exists for the CA.");
				throw fault;
			} else {
				return getPrivateKey(properties
