...
public class TrustServiceFileSystemCAManagerOnlyTest {
    ...
    public void testWithAndWithoutCRL() {
        try {
            ...
            Assert.assertFalse(isServerTrusted(ca1User1));
            Assert.assertFalse(isClientTrusted(ca1User1));

            writeTrustedCertificateAuthority("ABC", ca1);
            this.trustService.reloadTrustManagers();
            Assert.assertTrue(isServerTrusted(ca1User1));
            Assert.assertTrue(isClientTrusted(ca1User1));

            Credential ca2User1 = ca2.createIdentityCertificate("CA 123 User 1");
            Assert.assertFalse(isServerTrusted(ca2User1));
            Assert.assertFalse(isClientTrusted(ca2User1));

            writeTrustedCertificateAuthority("123", ca2);
            this.trustService.reloadTrustManagers();
            Assert.assertTrue(isServerTrusted(ca1User1));
            Assert.assertTrue(isServerTrusted(ca2User1));
            Assert.assertTrue(isClientTrusted(ca1User1));
            Assert.assertTrue(isClientTrusted(ca2User1));

            Credential ca2User2 = ca2.createIdentityCertificate("CA 123 User 2");
            Assert.assertTrue(isServerTrusted(ca1User1));
            Assert.assertTrue(isServerTrusted(ca2User1));
            Assert.assertTrue(isServerTrusted(ca2User2));
            Assert.assertTrue(isClientTrusted(ca1User1));
            Assert.assertTrue(isClientTrusted(ca2User1));
            Assert.assertTrue(isClientTrusted(ca2User2));
            ...
            Assert.assertTrue(isServerTrusted(ca1User1));
            Assert.assertTrue(isServerTrusted(ca2User1));
            Assert.assertFalse(isServerTrusted(ca2User2));
            Assert.assertTrue(isClientTrusted(ca1User1));
            Assert.assertTrue(isClientTrusted(ca2User1));
            Assert.assertFalse(isClientTrusted(ca2User2));
    ...
    private boolean isClientTrusted(Credential c) {
        return isClientTrusted(getChain(c));
    }
    ...
    private boolean isServerTrusted(Credential c) {
        return isServerTrusted(getChain(c));
    }
