...
public class DelegatedCredentialManager {
    ...
    public DelegatedCredentialManager(Database db, PropertyManager properties, KeyManager keyManager, List<PolicyHandler> policyHandlers,
            ProxyPolicy proxyPolicy, EventManager events) throws CDSInternalException {
        this.db = db;
        ...
        this.events = events;
        try {
            this.delegationAuditor = (EventAuditor) events.getEventHandler(DELEGATED_CREDENTIAL_AUDITOR);
        } catch (InvalidHandlerException e) {
            throw Errors.makeException(CDSInternalException.class,
                    "Could not initialize the delegation credential manager, could not obtain the delegated credential auditor.", e);
        }
        String currentKeyManager = properties.getKeyManager();
        if ((currentKeyManager != null) && (!currentKeyManager.equals(keyManager.getName()))) {
    ...
    public PolicyHandler findHandler(String policyClassName) throws InvalidPolicyException {
        ...
        for (int i = 0; i < handlers.size(); i++) {
            if (handlers.get(i).isSupported(policyClassName)) {
                if (!handlerFound) {
                    ...
                    throw Errors.makeException(InvalidPolicyException.class, Errors.MULTIPLE_HANDLERS_FOUND_FOR_POLICY + policyClassName
    ...
    public List<DelegatedCredentialAuditRecord> searchAuditLog(DelegatedCredentialAuditFilter f) throws CDSInternalException {
        try {
            String targetId = null;
            if (f.getDelegationIdentifier() != null) {
                targetId = String.valueOf(f.getDelegationIdentifier().getDelegationId());
            }
            ...
            List<Event> events = this.delegationAuditor.findEvents(targetId, f.getSourceGridIdentity(), eventType, start, end, null);
            ...
            for (int i = 0; i < events.size(); i++) {
                ...
                record.setEvent(DelegatedCredentialEvent.fromValue(events.get(i).getEventType()));
                record.setMessage(events.get(i).getMessage());
                record.setOccurredAt(events.get(i).getOccurredAt());
                record.setSourceGridIdentity(events.get(i).getReportingPartyId());
                records.add(record);
            }
    ...
    public List<DelegationRecord> findCredentialsDelegatedToClient(String callerIdentity, ClientDelegationFilter filter) throws CDSInternalException {
    ...
    public synchronized DelegationSigningRequest initiateDelegation(String callerGridIdentity, DelegationRequest request) throws CDSInternalException,
            DelegationException, InvalidPolicyException {
        this.buildDatabase();
        ...
        if ((request.getIssuedCredentialPathLength() < 0) || (this.proxyPolicy.getMaxDelegationPathLength() < request.getIssuedCredentialPathLength())) {
        ...
        try {
            ...
            PreparedStatement s = c.prepareStatement("INSERT INTO " + TABLE + " SET " + GRID_IDENTITY + "= ?, " + POLICY_TYPE + "= ?, " + STATUS + "= ?,"
                    + DATE_INITIATED + "=?," + DATE_APPROVED + "=?," + EXPIRATION + "=?," + DELEGATION_PATH_LENGTH + "=?," + PROXY_LIFETIME_HOURS + "=?,"
            ...
            KeyPair keys = this.keyManager.createAndStoreKeyPair(String.valueOf(delegationId), request.getKeyLength());
            ...
            logEvent(delegationId, callerGridIdentity, DelegatedCredentialEvent.DELEGATION_INITIATED, "Delegation initiated for " + callerGridIdentity + ".");
            return req;
    ...
    private void logEvent(long delegationId, String callerGridIdentity, DelegatedCredentialEvent event, String message) throws CDSInternalException {
        try {
            events.logEvent(String.valueOf(delegationId), callerGridIdentity, event.value(), message);
        } catch (Exception e) {
            throw Errors.makeException(CDSInternalException.class, "Unexpected error encountered in establishing the audit trail.", e);
        }
    }

    public boolean delegationExists(DelegationIdentifier id) throws CDSInternalException {
        try {
            if (id == null) {
                return false;
            } else {
    ...
    public DelegationRecord getDelegationRecord(DelegationIdentifier id) throws CDSInternalException, DelegationException {
        ...
        if (delegationExists(id)) {
            ...
            try {
                c = this.db.getConnection();
                PreparedStatement s = c.prepareStatement("select * from " + TABLE + " WHERE " + DELEGATION_ID + "= ?");
                ...
                if (rs.next()) {
                    ...
                    r.setDelegationStatus(DelegationStatus.fromValue(rs.getString(STATUS)));
                    r.setExpiration(rs.getLong(EXPIRATION));
                    r.setGridIdentity(rs.getString(GRID_IDENTITY));
                    r.setIssuedCredentialPathLength(rs.getInt(DELEGATION_PATH_LENGTH));
            ...
            try {
                X509Certificate[] certs = this.keyManager.getCertificates(String.valueOf(id.getDelegationId()));
                r.setCertificateChain(Utils.toCertificateChain(certs));
            } catch (Exception e) {
                log.error(e.getMessage(), e);
                throw Errors.makeException(CDSInternalException.class, Errors.UNEXPECTED_ERROR_LOADING_CERTIFICATE_CHAIN, e);
            }
            try {
                PolicyHandler handler = this.findHandler(getPolicyType(id.getDelegationId()));
                r.setDelegationPolicy(handler.getPolicy(id));
            } catch (Exception e) {
                log.error(e.getMessage(), e);
                throw Errors.makeException(CDSInternalException.class, Errors.UNEXPECTED_ERROR_LOADING_DELEGATION_POLICY, e);
            }
    ...
    public synchronized DelegationIdentifier approveDelegation(String callerGridIdentity, DelegationSigningResponse res) throws CDSInternalException,
        ...
        if (this.delegationExists(id)) {
            ...
            java.security.PublicKey publicKey = this.keyManager.getPublicKey(String.valueOf(id.getDelegationId()));
            ...
            try {
                if (CertUtil.isProxy(BouncyCastleUtil.getCertificateType(certs[0]))) {
                    int currLength = r.getIssuedCredentialPathLength();
                    for (int i = 0; i < certs.length; i++) {
                        if (CertUtil.isProxy(BouncyCastleUtil.getCertificateType(certs[i]))) {
                            int delegationPathLength = CertificateExtensionsUtil.getDelegationPathLength(certs[i]);
                ...
                try {
                    ...
                    validator.validate(certs, TrustedCertificates.getDefaultTrustedCertificates().getCertificates(),
                            CertificateRevocationLists.getDefaultCertificateRevocationLists());

                } catch (Exception e) {
                    log.debug(Errors.INVALID_CERTIFICATE_CHAIN, e);
                    throw Errors.makeException(DelegationException.class, Errors.INVALID_CERTIFICATE_CHAIN, e);
                }
                ...
                try {
                    if (!BouncyCastleUtil.getIdentity(certs).equals(r.getGridIdentity())) {
                        throw Errors.makeException(DelegationException.class, Errors.IDENTITY_DOES_NOT_MATCH_INITIATOR);
                    }
                } catch (CertificateException e) {
                    log.error(e.getMessage(), e);
                    throw Errors.makeException(CDSInternalException.class, Errors.UNEXPECTED_ERROR_EXTRACTING_IDENTITY_FROM_CERTIFICATE_CHAIN, e);
                }
                ...
                log.error(e.getMessage(), e);
                throw Errors.makeException(CDSInternalException.class, Errors.UNEXPECTED_ERROR_DETERMINING_DELEGATION_PATH_LENGTH, e);
            }

            this.keyManager.storeCertificates(String.valueOf(id.getDelegationId()), certs);
            Connection conn = null;
            try {
                Date now = new Date();
                conn = this.db.getConnection();
                PreparedStatement s = conn.prepareStatement("update " + TABLE + " SET " + STATUS + "=?," + EXPIRATION + "=?," + DATE_APPROVED + "=?"
                        + " WHERE " + DELEGATION_ID + "= ?");
            ...
            logEvent(id.getDelegationId(), callerGridIdentity, DelegatedCredentialEvent.DELEGATION_APPROVED, "The delegated credential for "
                    + callerGridIdentity + " has been approved.");
            return id;
        } else {
            throw Errors.makeException(DelegationException.class, Errors.DELEGATION_RECORD_DOES_NOT_EXIST);
        }
    ...
    public void updateDelegatedCredentialStatus(String callerGridIdentity, DelegationIdentifier id, DelegationStatus status) throws CDSInternalException,
            DelegationException {
        if (this.delegationExists(id)) {
            ...
            try {
                conn = this.db.getConnection();
                PreparedStatement s = conn.prepareStatement("update " + TABLE + " SET " + STATUS + "=?" + " WHERE " + DELEGATION_ID + "= ?");
            ...
            logEvent(id.getDelegationId(), callerGridIdentity, DelegatedCredentialEvent.DELEGATION_STATUS_UPDATED, "Delegation Status changed from "
                    + r.getDelegationStatus().value() + " to " + status.value());
        } else {
            throw Errors.makeException(DelegationException.class, Errors.DELEGATION_RECORD_DOES_NOT_EXIST);
        }
    ...
    public CertificateChain getDelegatedCredential(String gridIdentity, DelegationIdentifier id, PublicKey publicKey) throws CDSInternalException,
            DelegationException, PermissionDeniedException {
        if (delegationExists(id)) {
            DelegationRecord r = this.getDelegationRecord(id);

            if (!r.getDelegationStatus().equals(DelegationStatus.APPROVED)) {
                logEvent(id.getDelegationId(), gridIdentity, DelegatedCredentialEvent.DELEGATED_CREDENTIAL_ACCESS_DENIED, Errors.CANNOT_GET_INVALID_STATUS);
                throw Errors.makeException(DelegationException.class, Errors.CANNOT_GET_INVALID_STATUS);
            }

            PolicyHandler handler = null;
            try {
                handler = this.findHandler(r.getDelegationPolicy().getClass().getName());
            } catch (Exception e) {
                logEvent(id.getDelegationId(), gridIdentity, DelegatedCredentialEvent.DELEGATED_CREDENTIAL_ACCESS_DENIED, Errors.POLICY_HANDLER_NOT_FOUND);
                throw Errors.makeException(CDSInternalException.class, Errors.POLICY_HANDLER_NOT_FOUND, e);
            }
            if (!handler.isAuthorized(id, gridIdentity)) {
                logEvent(id.getDelegationId(), gridIdentity, DelegatedCredentialEvent.DELEGATED_CREDENTIAL_ACCESS_DENIED,
                        Errors.PERMISSION_DENIED_TO_DELEGATED_CREDENTIAL);
                throw Errors.makeException(PermissionDeniedException.class, Errors.PERMISSION_DENIED_TO_DELEGATED_CREDENTIAL);
            }
            Date now = new Date();
            Date expiration = new Date(r.getExpiration());
            if (now.after(expiration)) {
                logEvent(id.getDelegationId(), gridIdentity, DelegatedCredentialEvent.DELEGATED_CREDENTIAL_ACCESS_DENIED, Errors.SIGNING_CREDENTIAL_EXPIRED);
            ...
            try {
                ...
                c.add(Calendar.SECOND, r.getIssuedCredentialLifetime().getSeconds() + PROXY_EXPIRATION_BUFFER_SECONDS);
                ...
                X509Certificate[] proxy = ProxyCreator.createImpersonationProxyCertificate(certs,
                        this.keyManager.getPrivateKey(String.valueOf(id.getDelegationId())), pkey, hours, minutes, seconds, r.getIssuedCredentialPathLength());
                logEvent(id.getDelegationId(), gridIdentity, DelegatedCredentialEvent.DELEGATED_CREDENTIAL_ISSUED, "A credential was issued to " + gridIdentity
                        + ".  The credential will expire on " + proxy[0].getNotAfter().toString() + ".");
                return Utils.toCertificateChain(proxy);
            } catch (DelegationException f) {
                throw f;
            } catch (Exception e) {
                throw Errors.makeException(CDSInternalException.class, Errors.UNEXPECTED_ERROR_CREATING_PROXY, e);
            }
    ...
    public List<DelegationRecord> findDelegatedCredentials(DelegationRecordFilter f) throws CDSInternalException {
        this.buildDatabase();
        Connection c = null;
        try {
            c = this.db.getConnection();
            ...
            // NOTE: it appears like caGrid 1.4 (at least the gaards-ui) is sending a non-null delegationidentifier, which has a "long" that defaults to 0.
            // technically I should honor the request to look for delegationId=0, but since the ids actually start at 1, this would never return anything
            // anyway.
            // If you pass a 0, it will behave the same as if you passed null now.
            if (f.getDelegationIdentifier() != null && f.getDelegationIdentifier().getDelegationId() != 0) {
                if (!filterAdded) {
                    sql.append(" WHERE ");
                    filterAdded = true;
                } else {
            ...
            if (f.getGridIdentity() != null && !f.getGridIdentity().trim().equals("")) {
                if (!filterAdded) {
                    sql.append(" WHERE ");
                    filterAdded = true;
                } else {
            ...
            // see NOTE above where the WHERE/AND clause is added
            if (f.getDelegationIdentifier() != null && f.getDelegationIdentifier().getDelegationId() != 0) {
                count = count + 1;
    ...
    private String getPolicyType(long delegationId) throws CDSInternalException {
        ...
        try {
            c = this.db.getConnection();
            PreparedStatement s = c.prepareStatement("select " + POLICY_TYPE + " from " + TABLE + " WHERE " + DELEGATION_ID + "= ?");
    ...
    public synchronized void delete(DelegationIdentifier id) throws CDSInternalException {
    ...
    public synchronized void delete(long delegationId) throws CDSInternalException {
        ...
        try {
            List<Event> list = this.delegationAuditor.findEvents(String.valueOf(delegationId), null, null, null, null, null);
            for (int i = 0; i < list.size(); i++) {
                this.delegationAuditor.deleteEvent(list.get(i).getEventId());
            }
        } catch (Exception e) {
            log.error(e.getMessage(), e);
            throw Errors.makeException(CDSInternalException.class, "An inexpected error occurred in deleting the audit log.", e);
        }
    }
        ...
        if (!dbBuilt) {
            try {
                if (!this.db.tableExists(TABLE)) {
                    String trust = "CREATE TABLE " + TABLE + " (" + DELEGATION_ID + " BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY," + GRID_IDENTITY
                            + " VARCHAR(255) NOT NULL," + POLICY_TYPE + " VARCHAR(255) NOT NULL," + STATUS + " VARCHAR(50) NOT NULL," + DATE_INITIATED
                            + " BIGINT," + DATE_APPROVED + " BIGINT," + DELEGATION_PATH_LENGTH + " INT," + PROXY_LIFETIME_HOURS + " INT,"
                            + PROXY_LIFETIME_MINUTES + " INT," + PROXY_LIFETIME_SECONDS + " INT," + EXPIRATION + " BIGINT, INDEX document_index ("
                            + DELEGATION_ID + "));";
                    db.update(trust);
                }
